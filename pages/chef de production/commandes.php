...
function verify($id)
{
    ...
    $str = "Impossible de valider la production \n";
    $error = false;
    foreach ($commande_products as $cp) {
        $product = Database::select("product", "code", $cp['idProduit']);
        if ($product[0]['quantite'] < $cp['quantite']) {
            $str .= $cp['idProduit'] . ' est besion de ' . intval($cp['quantite']) - intval($product[0]['quantite']) . ' \n';
            $error = true;
        }
    }
    if ($error)
...
if (isset($_POST)) {
    if (isset($_POST['valider'])) {
        ...
        foreach ($commande_products as $cp) {
            ...
            Database::update("product", "code", $product[0]['code'], array("quantite"), array($product[0]['quantite'] - $cp['quantite']));
        }
        Database::update("commade", "idCommande", $_POST['valider'], array("done"), array("1"));
    }
}
                                                ...
                                                ?>
                                                    <tr>
                                                        ...
                                                        <?php if ($c['done'] == "0") : ?>
                                                            ...
                                                            <form method="POST" action="ohayo">
                                                                <?php
                                                                if (verify($c['idCommande']) != "") { ?>
                                                                    <td><button type="button" onclick="func<?php echo verify($c['idCommande']) ?>(" <?php echo verify($c['idCommande']) ?>")" class="btn btn-warning">Valider</button></td>
                                                                    <script>
                                                                        function func<?php echo verify($c['idCommande']) ?>(str) {
                                                                        }
                                                                    </script>
                                                                <?php } else { ?>
